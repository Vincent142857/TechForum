...
import { getAvatarOption, updateAvatarOption } from "../../../services/configService/configService";
...
const ConfigAvatar = () => {
  ...
  const editAvatarOption = async () => {
    ...
    let res = await updateAvatarOption(data, axiosJWT, currentUser?.accessToken);
    if (res && +res?.status === 200) {
      ...
      <section className="mb-3">
        {avatarOption != null ? (
          <ListGroup as="ol" numbered>
            <ListGroup.Item as="li">Max File Size: {avatarOption?.maxFileSize} KB</ListGroup.Item>
            <ListGroup.Item as="li">Max Height: {avatarOption?.maxHeight} px</ListGroup.Item>
            <ListGroup.Item as="li">Max width: {avatarOption?.maxWidth} px </ListGroup.Item>
          </ListGroup>
        ) : (
            ...
            <Card.Body>
              <Row>
                <Col>
                  ...
                  />
                  {errMaxSize && <small className="text-danger">{errMaxSize}</small>}
                </Col>
                <Col>
                  ...
                  />
                  {errHeight && <small className="text-danger">{errHeight}</small>}
                </Col>
                <Col>
                  ...
                  />
                  {errWidth && <small className="text-danger">{errWidth}</small>}
                </Col>
              </Row>
              <Row>
                <Col>
